...
export class GameplayManager extends EventManager {
	...
	constructor(
		scene: GameScene,
		name: string,
		previousPlayerPos?: ICoordinates
	) {
		super(scene, name);
	...
	public act(
		...
		if (!this.interacting) {
			if (
				keysPressed.includes(GameScene.interactKey) &&
				...
				this.scene.scene.launch('Dialogue', {
					dialogue: this.interactableObj.dialogue,
	...
	public interactionEnded() {
		...
		this.interacting = false;
		this.implementFlagChanges(
			this.interactableObj.getFlagsChangesAfterInteraction()
		);
	...
	private implementFlagChanges(flagChanges: IFlagChange[]) {
		...
		for (let flagChange of flagChanges) {
			...
			if (filterResult instanceof Error) {
				console.error(
					'Error in flag changes of interactable\n' + filterResult.message
				);
			} else {
	...
	public initPlayer(actorsArray: any): Player {
		let actor = actorsArray.find(
			(actor) => actor.id == this.jsonObj.player.actorId
		);
