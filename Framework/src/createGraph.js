...
function createGraph(jsonObj) {
	...
	const firstScenes = jsonObj['scenes'].filter(
		(scene) => jsonObj['firstLocation'] === scene['locationId']
	);
	if (firstScenes.length == 0) {
		let error =
			'No first location scenes: No scenes have a location id corresponding to the first location';
	...
	for (locationId of locationsIds) {
		...
		let locationScenes = jsonObj['scenes'].filter(
...
function getFirstEvents(errors, scene) {
	...
	for (eventName of firstEventsNames) {
		...
		if (event === undefined) {
			let error =
				'Undefined first event: First event ' +
				eventName +
				' in scene ' +
				scene['name'] +
				' is undefined';
...
function parseScene(
	...
	if (cyclePaths.length != 0) {
		for (cyclePath of cyclePaths) {
			addError(
				errors,
				'Cycle: There is a cycle in scene ' +
					scene['name'] +
					' with path: ' +
					cyclePath
			);
	...
	while (!stack.isEmpty()) {
		...
		if (isFirstEvent(currEvent, scene)) {
			let label = createLabel(
				currEvent['emotionalRequirements'],
				currEvent['priority']
			);
		...
		for (nextEvent of nextEvents) {
			...
			let label = createLabel(
				nextEvent['emotionalRequirements'],
				nextEvent['priority']
			);
...
function getNextEvents(errors, currEvent, events) {
	...
	for (eventName of currEvent['nextEvents']) {
		...
		if (event === undefined) {
			let error =
				'Undefined next event: Next event ' +
				eventName +
				' in ' +
				currEvent['name'] +
				' is undefined';
